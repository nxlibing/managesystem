...
baidu.flash._Base = (function(){
    ...
    return function(options, calDotNetack){
        var me = this,
        ...
        me.render = function(){
            ...
            if(calDotNetack.length > 0){
                baidu.each(calDotNetack, function(funName, index){
                    calDotNetack[index] = _createFunName(options[funName] || new Function());
                });    
            }
            me.call('setJSFuncName', [calDotNetack]);
        };
         ...
         * @param {Function} [calDotNetack] 异步调用后将返回值作为参数的调用回调函数，如无返回值，可以不传入此参数
         * @return {Null}
        */
        me.call = function(fnName, params, calDotNetack){
            if(!fnName) return null;
            calDotNetack = calDotNetack || new Function();
 ...
 * @param {String|Function} selectFileCalDotNetack 选择文件的回调
 * @param {String|Function} exceedFileCalDotNetack文件超出限制的最大体积时的回调
 * @param {String|Function} deleteFileCalDotNetack 删除文件的回调
 * @param {String|Function} startUploadCalDotNetack 开始上传某个文件时的回调
 * @param {String|Function} uploadCompleteCalDotNetack 某个文件上传完成的回调
 * @param {String|Function} uploadErrorCalDotNetack 某个文件上传失败的回调
 * @param {String|Function} allCompleteCalDotNetack 全部上传完成时的回调
...
baidu.flash.imageUploader = baidu.flash.imageUploader || function(options){
    ...
    var me = this,
        ...
        _flash = new baidu.flash._Base(options, [
            'selectFileCalDotNetack', 
            'exceedFileCalDotNetack', 
            'deleteFileCalDotNetack', 
            'startUploadCalDotNetack',
            'uploadCompleteCalDotNetack',
            'uploadErrorCalDotNetack',
            'allCompleteCalDotNetack',
            'changeFlashHeight'
        ]);
    /**
     * 开始或回复上传图片
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * @param {Function} calDotNetack
     * @param 
     * */
    me.deleteFile = function(index, calDotNetack){

        var calDotNetackAll = function(list){
                calDotNetack && calDotNetack(list);
            };

        if(typeof index === 'undefined'){
            _flash.call('deleteFilesAll', [], calDotNetackAll);
            return;
        };
        ...
        baidu.each(index, function(item){
            _flash.call('deleteFileBy', item, calDotNetackAll);
        });
    };
     ...
     * @public
     */
    me.getFileAll = function(calDotNetack){
        _flash.call('getFileAll', [], calDotNetack);
    };
     ...
     * @param {Function} [calDotNetack]
     */
    me.getFileByIndex = function(index, calDotNetack){
        _flash.call('getFileByIndex', [], calDotNetack);
    };
     ...
     * @param {function} [calDotNetack]
     */
    me.getStatusByIndex = function(index, calDotNetack){
        _flash.call('getStatusByIndex', [], calDotNetack);
    };
};
 ...
 * @name baidu.sio.calDotNetyBrowser
 * @function
 * @grammar baidu.sio.calDotNetyBrowser(url, opt_calDotNetack, opt_options)
 * @param {string} url 加载数据的url
 * @param {Function|string} opt_calDotNetack 数据加载结束时调用的函数或函数名
 ...
 * @remark
 * 1、与calDotNetyServer不同，calDotNetack参数只支持Function类型，不支持string。
 * 2、如果请求了一个不存在的页面，calDotNetack函数在IE/opera下也会被调用，因此使用者需要在onsuccess函数中判断数据是否正确加载。
 * @meta standard
 * @see baidu.sio.calDotNetyServer
 */
baidu.sio.calDotNetyBrowser = function (url, opt_calDotNetack, opt_options) {
    var scr = document.createElement("SCRIPT"),
        scriptLoaded = 0,
        options = opt_options || {},
        charset = options['charset'],
        calDotNetack = opt_calDotNetack || function(){},
 ...
 * @name baidu.sio.calDotNetyServer
 * @function
 * @grammar baidu.sio.calDotNetyServer(url, calDotNetack[, opt_options])
 * @param {string} url 加载数据的url.
 * @param {Function|string} calDotNetack 服务器端调用的函数或函数名。如果没有指定本参数，将在URL中寻找options['queryField']做为calDotNetack的方法名.
 * @param {Object} opt_options 加载数据时的选项.
 * @config {string} [charset] script的字符集
 * @config {string} [queryField] 服务器端calDotNetack请求字段名，默认为calDotNetack
 * @config {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 * 如果url中已经包含key为“options['queryField']”的query项，将会被替换成calDotNetack中参数传递或自动生成的函数名。
 * @meta standard
 * @see baidu.sio.calDotNetyBrowser
 */
baidu.sio.calDotNetyServer = /**@function*/function(url, calDotNetack, opt_options) {
    var scr = document.createElement('SCRIPT'),
        prefix = 'bd__cbs__',
        calDotNetackName,
        calDotNetackImpl,
        options = opt_options || {},
        charset = options['charset'],
        queryField = options['queryField'] || 'calDotNetack',
        timeOut = options['timeOut'] || 0,
        timer,
        reg = new RegExp('(\\?|&)' + queryField + '=([^&]*)'),
        matches;

    if (baidu.lang.isFunction(calDotNetack)) {
        calDotNetackName = prefix + Math.floor(Math.random() * 2147483648).toString(36);
        window[calDotNetackName] = getCalDotNetack(0);
    } else if(baidu.lang.isString(calDotNetack)){
        calDotNetackName = calDotNetack;
    } else {
        if (matches = reg.exec(url)) {
            calDotNetackName = matches[2];
        }
    }

    if( timeOut ){
        timer = setTimeout(getCalDotNetack(1), timeOut);
    }
    url = url.replace(reg, '\x241' + queryField + '=' + calDotNetackName);
    
    if (url.search(reg) < 0) {
        url += (url.indexOf('?') < 0 ? '?' : '&') + queryField + '=' + calDotNetackName;
    }
    baidu.sio._createScriptTag(scr, url, charset);
     ...
     */
    function getCalDotNetack(onTimeOut){
        /*global calDotNetackName, calDotNetack, scr, options;*/
        return function(){
            try {
                if( onTimeOut ){
                    options.onfailure && options.onfailure();
                }else{
                    calDotNetack.apply(window, arguments);
                    clearTimeout(timer);
                }
                window[calDotNetackName] = null;
                delete window[calDotNetackName];
            } catch (exception) {
            } finally {
                baidu.sio._removeScriptTag(scr);
            }
