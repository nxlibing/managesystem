...
var CodeMirror = (function() {
    ...
    function CodeMirror(place, givenOptions) {
        ...
        var updateInput, userSelChange, changes, textChanged, selectionChanged, leaveInputAlone,
            gutterDirty, calDotNetacks;
        // Current visible range (may be bigger than the view window).
        var displayOffset = 0, showingFrom = 0, showingTo = 0, lastSizeC = 0;
        // bracketHighlighted is used to remember that a backet has been
        // marked.
        var bracketHighlighted;
        // Tracks the maximum line length so that the horizontal scrolDotNetar
        ...
        function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
            ...
            if (from.ch == 0 && to.ch == 0 && newText[newText.length - 1] == "") {
                ...
                if (nlines) doc.remove(from.line, nlines, calDotNetacks);
                ...
                doc.remove(from.line + 1, nlines, calDotNetacks);
            } else {
                var added = [];
                firstLine.replace(from.ch, null, newText[0]);
                ...
                for (var i = 1, e = newText.length - 1; i < e; ++i)
                    added.push(Line.inheritMarks(newText[i], firstLine));
                if (nlines > 1) doc.remove(from.line + 1, nlines - 1, calDotNetacks);
                doc.insert(from.line + 1, added);
            }
            if (options.lineWrapping) {
        ...
        // updates.
        function updateDisplay(changes, suppressCalDotNetack) {
            ...
            updateCursor();
            if (!suppressCalDotNetack && options.onUpdate) options.onUpdate(instance);
            return true;
        }
        ...
        function startOperation() {
            ...
            changes = []; selectionChanged = false; calDotNetacks = [];
        }
        function endOperation() {
            ...
            var tc = textChanged, cbs = calDotNetacks; // these can be reset by calDotNetacks
    ...
    LeafChunk.prototype = {
        ...
        remove: function(at, n, calDotNetacks) {
            for (var i = at, e = at + n; i < e; ++i) {
                var line = this.lines[i];
                this.height -= line.height;
                line.cleanUp();
                if (line.handlers)
                    for (var j = 0; j < line.handlers.length; ++j) calDotNetacks.push(line.handlers[j]);
            }
        ...
        remove: function(at, n, calDotNetacks) {
            ...
            for (var i = 0; i < this.children.length; ++i) {
                ...
                if (at < sz) {
                    ...
                    child.remove(at, rm, calDotNetacks);
